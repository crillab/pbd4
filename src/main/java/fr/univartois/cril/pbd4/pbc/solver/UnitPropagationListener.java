 ...
 * @version 0.2.0
 */
public final class UnitPropagationListener extends SearchListenerAdapter<ISolverService> {
    ...
    public void enqueueing(int p, IConstr reason) {
        if ((reason != null) && (Math.abs(p) <= numberOfVariables)) {
            // This literal has been propagated and is NOT a selector, so we record it.
